<script lang="ts">
    ...
    async function handleExport() {
        try {
            await downloadData(<string>$clientId, localDecimation);
        } catch (error) {
...
<footer class="fixed w-full bottom-0 left-0 bg-white border-t border-gray-200 px-4 md:px-6 py-3 md:py-4">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 md:gap-4">
            <div class="flex-1">
                {#if typeof $connectionDate === "number" && $connectionState === 'connected'}
                    <p class="text-xs md:text-sm font-medium text-gray-700">
                        Datos registrados desde: {formatTimestamp($connectionDate)}
                    </p>
                {:else }
            ...
            <div class="flex items-center gap-2 md:gap-4 w-full md:w-auto justify-center md:justify-around">
                <div>
